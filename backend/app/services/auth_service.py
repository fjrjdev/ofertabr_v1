...
class AuthService:
    ...
    async def verify_access_code(self, email: str, code: str) -> str | None:
        cache_key = f"auth:code:{email}"
    ...
    async def verify_magic_token(self, token: str) -> str | None:
        cache_key = f"auth:magic:{token}"
        email = await cache_service.get(cache_key)

        if not email:
            logger.warning("Invalid or expired magic token")
            return None
